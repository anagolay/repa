...
async function generate(
  ...
  if (!equals(yamlFile.cme, 'docker')) {
    throw new Error(`Unsupported engine: ${yamlFile.cme}. Currently supported engines are [docker]`);
  }
  ...
  const relayFileNameSpecPlain = generateSpecFileName(yamlFile.relay.spec.chainType);

  await writeFile(resolve(process.cwd(), output, relayFileNameSpecPlain), plainSpecModified);

  const rawSpec = await buildSpecRaw(yamlFile, relayFileNameSpecPlain, output);
  const relayFileNameSpecRaw = generateSpecFileName(yamlFile.relay.spec.chainType, true);
  await writeFile(resolve(process.cwd(), output, relayFileNameSpecRaw), rawSpec);
  ...
  console.log(
    `All files are written, please run the containers from the output folder then run ${chalk.green(
      'insert-keys'
    )} command.`
  );
}
 ...
 */
async function writeFile<T>(filePath: string, data: T, asYaml: boolean = false): Promise<void> {
  const encoded = asYaml ? yaml.stringify(data, null, 2) : JSON.stringify(data, null, 2);
